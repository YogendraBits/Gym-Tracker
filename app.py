...
def main_app():
    ...
    # Body Metrics Page
        ...
        # Create tabs
        tab1, tab2, tab3 = st.tabs(["📝 Log Metrics", "📊 Progress Charts", "📋 History"])
        ...
        with tab1:
            ...
            with col1:
                ...
                st.markdown('<h3 class="section-title">📅 Basic Info</h3>', unsafe_allow_html=True)
                
                metrics_date = st.date_input("Date", value=date.today(), key="metrics_date")
                weight = st.number_input("Weight (kg)", min_value=30.0, max_value=200.0, step=0.1, key="weight_input")
                
                # Body fat percentage (optional)
                body_fat = st.number_input("Body Fat % (optional)", min_value=5.0, max_value=50.0, step=0.1, key="body_fat")
                
                st.markdown('</div>', unsafe_allow_html=True)
                
            with col2:
                st.markdown('<div class="form-section">', unsafe_allow_html=True)
                st.markdown('<h3 class="section-title">📐 Measurements (cm)</h3>', unsafe_allow_html=True)
            ...
            # Notes section
            st.markdown('<div class="form-section full-width">', unsafe_allow_html=True)
            st.markdown('<h3 class="section-title">📝 Notes</h3>', unsafe_allow_html=True)
            metrics_notes = st.text_area("Add any notes about your measurements", placeholder="e.g., Measured after workout, morning weight, etc.", key="metrics_notes")
            st.markdown('</div>', unsafe_allow_html=True)
        ...
        with tab2:
            ...
            time_period = st.selectbox("Select Time Period", ["Last 30 Days", "Last 90 Days", "Last 6 Months", "Last Year"], index=1)
            ...
            if metrics_data:
                ...
                measurement_columns = [col for col in df.columns if col.startswith('measurement_')]
                if measurement_columns:
                    st.markdown('<div class="chart-container">', unsafe_allow_html=True)
                    
                    # Create measurement trends
                    measurement_df = df[['date'] + measurement_columns].copy()
                    measurement_df_melted = measurement_df.melt(id_vars=['date'], var_name='measurement', value_name='value')
                    measurement_df_melted['measurement'] = measurement_df_melted['measurement'].str.replace('measurement_', '').str.title()
                ...
                st.markdown('<h3>📈 No Data Available</h3>', unsafe_allow_html=True)
                st.markdown('<p>Start logging your body metrics to see progress charts here!</p>', unsafe_allow_html=True)
                st.markdown('</div>', unsafe_allow_html=True)
        ...
        with tab3:
            ...
            metrics_data = db.get_body_metrics_data(365)  # Get full year of data
            ...
            if metrics_data:
                ...
                if len(df) > 0:
                    ...
                    st.markdown('<h3 class="section-title">📊 Latest Measurements</h3>', unsafe_allow_html=True)
                    ...
                    if measurements:
                        st.markdown('<h4 class="subsection-title">Body Measurements</h4>', unsafe_allow_html=True)
                ...
                st.markdown('<h3 class="section-title">📋 Measurement History</h3>', unsafe_allow_html=True)
                
                # Format data for display
                display_df = df.copy()
                display_df['Date'] = display_df['date'].dt.strftime('%Y-%m-%d')
                display_df = display_df[['Date', 'weight'] + [col for col in display_df.columns if col.startswith('measurement_')]]
                
                # Rename columns
                column_mapping = {'weight': 'Weight (kg)'}
                for col in display_df.columns:
                    if col.startswith('measurement_'):
                        column_mapping[col] = col.replace('measurement_', '').title() + ' (cm)'
                ...
                st.markdown('<div class="empty-state">', unsafe_allow_html=True)
                st.markdown('<h3>📋 No History Available</h3>', unsafe_allow_html=True)
                st.markdown('<p>Your measurement history will appear here once you start logging data.</p>', unsafe_allow_html=True)
                st.markdown('</div>', unsafe_allow_html=True)
